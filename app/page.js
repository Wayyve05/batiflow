  ...
  return <div style={{background:C.w50}}>
    ...
    <section id="tarifs" style={{background:C.w50,padding:mob?"48px 16px":"80px 32px",textAlign:"center"}}><div style={{maxWidth:1200,margin:"0 auto"}}>
      ...
      <div style={{maxWidth:420,margin:"0 auto",borderRadius:20,padding:32,border:`2px solid ${C.g500}`,background:C.g900,color:C.wh,position:"relative"}}>
        ...
        <div style={{display:"flex",alignItems:"baseline",gap:3,justifyContent:"center",marginBottom:4}}><span style={{fontSize:"2.8rem",fontWeight:800,fontFamily:"'Playfair Display',Georgia,serif"}}>29,99</span><span style={{fontSize:"1rem",color:"rgba(255,255,255,0.5)"}}>EUR/mois</span></div>
        <div style={{textAlign:"center",fontSize:"0.85rem",color:C.g300,marginBottom:20}}>ou 299,99EUR/an (2 mois offerts)</div>
...
function Dash({art,setArt,onOut,email,uid}){
  ...
  const main=<main style={{padding:mob?16:24,background:C.x100,overflowY:"auto",paddingTop:mob?60:24,minHeight:"100vh"}}>
    ...
    {v==="view-d"&&cur&&<DocV doc={cur} a={art} type="devis" uid={uid} mob={mob} onBack={()=>go("devis-list")} onEdit={()=>setV("edit-d")} onFac={()=>{const now=new Date();const ech=new Date(now.getTime()+30*24*60*60*1000);const f2={...cur,id:Date.now(),date:now.toLocaleDateString("fr-FR"),echeance:ech.toLocaleDateString("fr-FR"),status:"Emise",num:`FAC-${new Date().getFullYear()}-${String(facs.length+1).padStart(4,"0")}`};setFacs(p=>[f2,...p]);saveFac(f2);setDevis(p=>p.map(d=>d.id===cur.id?{...d,status:"Accepte"}:d));setCur(f2);setV("view-f");toast("Facture creee !")}}/>}
    ...
    {v==="view-f"&&cur&&<DocV doc={cur} a={art} type="facture" uid={uid} mob={mob} onBack={()=>go("factures")} onPay={()=>{setFacs(p=>p.map(f=>f.id===cur.id?{...f,status:"Payee"}:f));setCur({...cur,status:"Payee"});toast("Facture encaissee !")}}/>}
    {v==="clients"&&<CliV clis={clis} setClis={setClis} mob={mob} devis={devis} facs={facs} onVD={d=>{setCur(d);setV("view-d")}} onVF={f=>{setCur(f);setV("view-f")}} chantiers={chantiers} onVCh={id=>{setSelCh(id);go("chantiers")}}/>}
...
function HView({d,f,a,go,onNew,onVD,mob}){
  return <div>
    ...
    {(()=>{const now=new Date();const relances=d.filter(x=>{if(x.status!=="Brouillon")return false;const parts=(x.date||"").split("/");if(parts.length!==3)return false;const dt=new Date(parts[2],parts[1]-1,parts[0]);return(now-dt)/(1000*60*60*24)>3});return relances.length>0?<div style={{...crd,marginBottom:14,borderLeft:"4px solid "+C.o500}}><h3 style={{fontWeight:700,color:C.o500,marginBottom:8,fontSize:"0.9rem"}}>⏰ Devis a relancer</h3>{relances.map((x,i)=><div key={i} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"6px 0",fontSize:"0.84rem"}}><span onClick={()=>onVD(x)} style={{cursor:"pointer",fontWeight:600}}>{x.num} — {x.clientNom}</span><span style={{fontSize:"0.72rem",color:C.o500}}>{x.totalTTC}</span></div>)}</div>:null})()}
    <div style={crd}>
...
// DOC VIEW
function DocV({doc,a,type,mob,onBack,onEdit,onFac,onPay,onSent,uid}){
  const toast=useT();const ref=useRef();const isD=type==="devis";const T=isD?"DEVIS":"FACTURE";
  const[sending,setSending]=useState(false);const[emailModal,setEmailModal]=useState(false);const[toEmail,setToEmail]=useState(doc.clientEmail||"");const[sigModal,setSigModal]=useState(false);const[signature,setSignature]=useState(doc.signature||null);const sigRef=useRef(null);const sigCtx=useRef(null);const drawing=useRef(false);const[relance,setRelance]=useState("none");
  const initSig=()=>{const c=sigRef.current;if(!c)return;c.width=360;c.height=160;const ctx=c.getContext("2d");ctx.strokeStyle="#1a1a1a";ctx.lineWidth=2;ctx.lineCap="round";sigCtx.current=ctx};
  const startDraw=(e)=>{drawing.current=true;const c=sigRef.current;const rect=c.getBoundingClientRect();const x=(e.touches?e.touches[0].clientX:e.clientX)-rect.left;const y=(e.touches?e.touches[0].clientY:e.clientY)-rect.top;sigCtx.current.beginPath();sigCtx.current.moveTo(x,y)};
  const draw=(e)=>{if(!drawing.current)return;e.preventDefault();const c=sigRef.current;const rect=c.getBoundingClientRect();const x=(e.touches?e.touches[0].clientX:e.clientX)-rect.left;const y=(e.touches?e.touches[0].clientY:e.clientY)-rect.top;sigCtx.current.lineTo(x,y);sigCtx.current.stroke()};
  const endDraw=()=>{drawing.current=false};
  const saveSig=()=>{const data=sigRef.current.toDataURL();setSignature(data);setSigModal(false)};
  ...
  const sendEmail=async()=>{if(!toEmail){toast("Entrez un email","error");return}setSending(true);try{const r=await fetch("/api/send-email",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({to:toEmail,type:T,num:doc.num,clientNom:doc.clientNom,clientAdresse:doc.clientAdresse||"",entreprise:a.entreprise,totalTTC:doc.totalTTC,lignes:doc.lignes,description:doc.description,conditions:doc.conditions,tvaRate:doc.tvaRate,totalHT:doc.totalHT,tva:doc.tva,artisan:a,devisId:doc.id,relance:relance,uid:uid||""})});const data=await r.json();if(data.error)throw new Error(data.error);toast("Email envoye !");setEmailModal(false)}catch(e){toast("Erreur envoi email","error")}setSending(false)};
  return <div>
    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:16,flexWrap:"wrap"}}>
      ...
      <div style={{display:"flex",gap:6,marginLeft:mob?0:"auto",flexWrap:"wrap"}}>
        ...
        {isD&&<button style={{...bs,...bsm}} onClick={()=>{setSigModal(true);setTimeout(initSig,100)}}>✍️ Signer</button>}
    ...
    {sigModal&&<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.5)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}><div style={{background:C.wh,borderRadius:16,padding:24,maxWidth:420,width:"100%"}}><h3 style={{fontWeight:700,color:C.g900,marginBottom:12}}>Signature client</h3><canvas ref={sigRef} onMouseDown={startDraw} onMouseMove={draw} onMouseUp={endDraw} onMouseLeave={endDraw} onTouchStart={startDraw} onTouchMove={draw} onTouchEnd={endDraw} style={{border:"2px solid "+C.x200,borderRadius:8,width:"100%",height:160,touchAction:"none",cursor:"crosshair"}}/><div style={{display:"flex",gap:8,marginTop:12}}><button style={{...bp,flex:1,justifyContent:"center"}} onClick={saveSig}>Valider</button><button style={{...bs,flex:1,justifyContent:"center"}} onClick={()=>{const ctx=sigRef.current.getContext("2d");ctx.clearRect(0,0,360,160)}}>Effacer</button><button style={{...bs,justifyContent:"center"}} onClick={()=>setSigModal(false)}>Annuler</button></div></div></div>}
    {emailModal&&<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.5)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}><div style={{background:C.wh,borderRadius:16,padding:24,maxWidth:400,width:"100%"}}><h3 style={{fontWeight:700,color:C.g900,marginBottom:12}}>Envoyer par email</h3><div style={{marginBottom:12}}><span style={lbl}>Email du client</span><input style={inp} type="email" placeholder="client@email.com" value={toEmail} onChange={e=>setToEmail(e.target.value)}/></div>{isD&&<div style={{marginBottom:12}}><span style={lbl}>Relance automatique</span><div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{[{v:"none",l:"Pas de relance"},{v:"3",l:"Sous 3 jours"},{v:"7",l:"Sous 7 jours"},{v:"14",l:"Sous 14 jours"}].map(o=><button key={o.v} onClick={()=>setRelance(o.v)} style={{padding:"6px 12px",borderRadius:50,fontSize:"0.75rem",fontWeight:600,border:`1.5px solid ${relance===o.v?C.g500:C.x200}`,background:relance===o.v?C.g100:C.wh,color:relance===o.v?C.g700:C.x500,cursor:"pointer",fontFamily:"inherit"}}>{o.l}</button>)}</div></div>}<div style={{display:"flex",gap:8}}><button style={{...bp,flex:1,justifyContent:"center",opacity:sending?0.6:1}} onClick={sendEmail} disabled={sending}>{sending?"Envoi...":"Envoyer"}</button><button style={{...bs,flex:1,justifyContent:"center"}} onClick={()=>setEmailModal(false)}>Annuler</button></div></div></div>}
    <div ref={ref} style={{background:C.wh,borderRadius:16,padding:mob?20:32,boxShadow:"0 4px 20px rgba(0,0,0,0.05)",border:`1px solid ${C.g100}`,maxWidth:780,margin:"0 auto"}}>
      ...
      <div style={{marginTop:20,padding:14,background:C.g50,borderRadius:10,fontSize:"0.78rem",color:C.x700,lineHeight:1.6}}>
        <strong>Conditions:</strong> {doc.conditions}<br/><div style={{marginTop:8,fontSize:"0.72rem",color:C.x500,lineHeight:1.6}}>{a.assurance&&<div>Assurance decennale: {a.assurance}{a.assureur&&" - "+a.assureur}{a.couverture&&" - Couverture: "+a.couverture}</div>}{a.siret&&<div>SIRET: {a.siret}</div>}<div>TVA non applicable, art. 293 B du CGI</div>{isD&&<div>Devis gratuit - Valable 3 mois</div>}{!isD&&<div>Penalites de retard: 3 fois le taux d interet legal. Indemnite forfaitaire de recouvrement: 40 EUR.</div>}<div>En cas de litige, le client peut recourir a un mediateur de la consommation.</div></div>{isD&&<><div style={{marginTop:14,borderTop:`1px dashed ${C.x300}`,paddingTop:10}}><div style={{fontSize:"0.72rem",color:C.x500,marginBottom:4}}>Droit de retractation: le client dispose de 14 jours pour se retracter si le devis est signe hors etablissement.</div><div style={{fontSize:"0.72rem",color:C.x500,marginBottom:8}}>Bon pour accord :</div><div style={{display:"flex",justifyContent:"flex-end",flexWrap:"wrap",gap:8,alignItems:"flex-end"}}>{signature?<div><div style={{fontSize:"0.7rem",color:C.x500}}>Signature:</div><img src={signature} style={{maxHeight:60,border:"1px solid "+C.x200,borderRadius:4}}/></div>:<span>Signature: ___________</span>}</div></div></>}
      </div>
    </div>
  </div>;
}
...
// CLIENTS
function CliV({clis,setClis,mob,devis,facs,onVD,onVF,chantiers,onVCh}){const toast=useT();const[add,setAdd]=useState(false);const[q,setQ]=useState("");
...
function ChantierV({mob,chantiers,setChantiers,clis,initSel,onClearSel}){
  const toast=useT();const[adding,setAdding]=useState(false);const[sel,setSel]=useState(initSel||null);
...
function SetV({art,setArt,mob}){const toast=useT();const[f,sF]=useState({...art});const u=(k,v)=>sF(p=>({...p,[k]:v}));
  return <div><h1 style={{fontFamily:"'Playfair Display',Georgia,serif",fontSize:"1.4rem",color:C.g900,marginBottom:14}}>Mon entreprise</h1>
    <div style={crd}>
      ...
      <div style={{display:"grid",gridTemplateColumns:mob?"1fr":"1fr 1fr",gap:8}}>
        {[["entreprise","Entreprise"],["siret","SIRET"],["adresse","Adresse"],["ville","Ville"],["telephone","Tel"],["email","Email"],["assurance","Assurance (n police)"],["assureur","Nom assureur"],["couverture","Zone couverte"]].map(([k,l])=><div key={k}><span style={lbl}>{l}</span><input style={inp} value={f[k]||""} onChange={e=>u(k,e.target.value)}/></div>)}
    ...
    <div style={{display:"grid",gridTemplateColumns:mob?"1fr":"1fr 1fr",gap:16,maxWidth:700}}>
      <div style={{...crd,border:`2px solid ${C.g500}`,position:"relative"}}>
        ...
        <div style={{textAlign:"center",marginBottom:16,marginTop:8}}>
          <div style={{fontSize:"2.2rem",fontWeight:800,color:C.g800,fontFamily:"'Playfair Display',Georgia,serif"}}>29,99<span style={{fontSize:"1rem",fontWeight:500}}>EUR/mois</span></div>
          <div style={{fontSize:"0.82rem",color:C.x500,marginTop:4}}>ou <strong style={{color:C.g700}}>299,99 EUR/an</strong> (2 mois offerts)</div>
        </div>
