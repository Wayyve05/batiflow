...
export async function GET(req) {
  ...
  if (id) {
    ...
    const { data } = await supabase.from('devis').select('*').eq('numero', num).eq('user_id', uid).order('created_at', { ascending: false }).limit(1)
    devis = data && data[0] ? data[0] : null
  }

  if (!devis) {
    return new Response('<!DOCTYPE html><html><head><meta charset="utf-8"></head><body style="font-family:-apple-system,sans-serif;padding:40px;text-align:center"><h1 style="color:#145A3E">Document introuvable</h1><p style="color:#666;margin-top:12px">Ce lien n est plus valide ou le document a ete supprime.</p></body></html>', {
  ...
  const { data: profile } = await supabase.from('profiles').select('*').eq('id', devis.user_id).single()
  const ent = profile?.entreprise || ''
  const accepted = devis.status === 'Accepte'
  const linkBase = '?id=' + devis.id
  ...
  const date = devis.created_at ? new Date(devis.created_at).toLocaleDateString("fr-FR") : ''
  ...
  const html = '<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Devis ' + devis.numero + ' - ' + ent + '</title>' + printStyle + '</head><body style="font-family:-apple-system,Helvetica,sans-serif;margin:0;padding:0;background:#f5f5f5"><div style="max-width:700px;margin:0 auto;padding:20px">'
    + '<div class="no-print" style="display:flex;gap:8px;margin-bottom:16px;justify-content:center;flex-wrap:wrap">'
    + (accepted ? '' : '<a href="' + linkBase + '&action=accept" style="display:inline-block;background:#145A3E;color:white;padding:12px 28px;border-radius:8px;text-decoration:none;font-weight:700;font-size:14px">Accepter ce devis</a>')
    + '<a href="' + linkBase + '&print=1" style="display:inline-block;background:#333;color:white;padding:12px 28px;border-radius:8px;text-decoration:none;font-weight:700;font-size:14px">Telecharger PDF</a>'
    + '</div>'
    ...
    + '<div style="text-align:right"><h1 style="font-size:22px;color:#145A3E;margin:0">DEVIS</h1><span style="font-size:12px;color:#737373">N. ' + devis.numero + '<br/>' + date + '</span></div></div>'
    ...
    + (profile?.assurance ? '<div style="margin-top:8px;font-size:10px;color:#999">Assurance decennale: ' + profile.assurance + (profile.assureur ? ' - ' + profile.assureur : '') + (profile.couverture ? ' - Couverture: ' + profile.couverture : '') + '</div>' : '')
    + '<div style="margin-top:8px;font-size:10px;color:#999">' + (profile?.siret ? 'SIRET: ' + profile.siret + ' - ' : '') + 'TVA non applicable, art. 293 B du CGI</div>'
    + '<div style="margin-top:8px;font-size:10px;color:#999">Devis gratuit - Valable 3 mois</div>'
